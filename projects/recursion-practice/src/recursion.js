...
var createArray = function(str){
  var newArray = [];
    ...
    newArray = newArray.concat(str[0]);
    return newArray = newArray.concat(createArray(str.slice(1)));
  };
...
var capitalizeFirst = function(array) {
  ...
  var letters = array[0].split('');
  letters[0] = letters[0].toUpperCase();
  
  return [letters.join('')].concat(capitalizeFirst(array.slice(1)));
};
...
// alternateSign([-2,-7,8,3,-1,4]) // [2,-7,8,-3,1,-4]
var alternateSign = function(array, counter = 0, sign = 1) {
  ...
  if(sign === 1) {
    ...
    return alternateSign(array, counter + 1, 0);
  } else {
    if(array[counter] > 0) {
      array[counter] *= -1;
    }
    return alternateSign(array, counter + 1, 1);
  }
};
...
var numToText = function(str, value = '') {
  ...
  if (Number(str[0]) && str[0] !== ' ') {
    return numToText(str.slice(1), value + text[Number(str[0])]);
  }
